<?php
    ...
    * 																		*
    *************************************************************************
	* master/reportSales.php :: Master Report Sales Page					*
	****************************************************************************
	* The sales report page for master									*
	*															*
	* Version			: 1											*
	...
	***************************************************************************/
	...
	//*** BEGIN PAGE PROCESSING *************************************************//
		...
		if ( count($_POST) > 0 ) //$_POST is always set, so we check by # of element
		{
			...
			{
				$cSales->Verify($_POST['salesVerifyID'], $_POST['salesVerifyNotes']);
			}
		}
		else
		{
	...
	$websiteFiles = array(
		...
		"content" => "master/reportSales.htm"
	);
	...
	$cWebsite->template->set_var(array(
		...
		"VAR_FORMACTION" => "master/reportSales.php",
		"VAR_FORMPRINT" => "master/reportSalesPrint.php",
		"VAR_FORMSAVE" => "master/reportSalesSave.php",
	...
	for ($i = 0; $i < count($aSalesList); $i++)
	{
		...
		if ($iOldSalesID == $aSalesList[$i]['sales_ID'])
		{
		...
		$iOldSalesID = $aSalesList[$i]['sales_ID'];

		//check for the next record to check if it is still the same sales_ID
		$bShow = FALSE;
		if ( ( ($i+1) == count($aSalesList)) || $iOldSalesID <> $aSalesList[$i+1]['sales_ID'])
		{
			$bShow = TRUE;
		}
