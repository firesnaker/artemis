<?php
    ...
    * 																		*
    *************************************************************************
	* finance/sales_mkios.php :: Finance Sales MKIOS Verify Page								*
	*********************************************************************
	* The sales verify page for mkios											*
	*																	*
	* Version		: 0.1												*
	* Author		: FireSnakeR 										*
	* Created		: 2014-02-14 										*
...
set_time_limit(0);
	...
	$websiteFiles = array(
		...
		"content" => "finance/sales_mkios.htm"
	);
	...
	$cWebsite->template->set_var(array(
		...
		"VAR_FORMURL" => "finance/sales_mkios.php"
	));
	...
	{
		//save to file
		$fp = fopen($sAbsolutePath . "/pdf/mkiosSales.csv", "w");
		fwrite($fp, "KodeWH; KodeSales; CustomerGroup; NamaCust; TxPeriod; KodeTerminal; NoHP; Subtotal; S005; S010; S020; S025; S050; S100; FinanceStatus; FinanceNotes;\r\n");
		for ($i = 0; $i < count($reportListBlock); $i++)
		{
			fwrite($fp, 
				'"' . $reportListBlock[$i]["VAR_KODEWH"] . '";'
				. '"' . $reportListBlock[$i]["VAR_KODESALES"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_CUSTOMERGROUP"] . '";'
				. '"' . $reportListBlock[$i]["VAR_NAMACUST"] . '";'
				. '"' . $reportListBlock[$i]["VAR_TXPERIOD"] . '";'
				. '"' . $reportListBlock[$i]["VAR_KODETERMINAL"] . '";'
				. '"' . $reportListBlock[$i]["VAR_NOHP"] . '";'
				. '"' . $reportListBlock[$i]["VAR_SUBTOTAL"] . '";'
				. '"' . $reportListBlock[$i]["VAR_S005_SAVE"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_S010_SAVE"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_S020_SAVE"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_S025_SAVE"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_S050_SAVE"] . '";' 
				. '"' . $reportListBlock[$i]["VAR_S100_SAVE"] . '";'
				. '"' . $reportListBlock[$i]["VAR_FINANCESTATUS"] . '";'
				. '"' . $reportListBlock[$i]["VAR_FINANCEVERIFYNOTES"] . '"'
				. "\r\n");
		}
		fwrite($fp, ";;;;;;Grandtotal;\"" . $iGrandSubtotal . "\";\"" . $iS005 . "\";\"" . $iS010 . "\";\"" . $iS020 . "\";\"" . $iS025 . "\";\"" . $iS050 . "\";\"" . $iS100 . "\";;\r\n");
		fclose($fp);

		//send out the file for download
		...
		*/
		set_time_limit(0);
		$file_path = $sAbsolutePath . "/pdf/mkiosSales.csv";
		output_file($file_path, 'mkiosSales-' . date("d-M-Y") . '.csv', 'text/plain');
	}
