<?php
    ...
    * 																		*
    *************************************************************************
	* finance/depositBank.php :: Finance Deposit Bank Page								*
	*********************************************************************
	* The deposit bank page for finance											*
	*																	*
	* Version		: 0.1												*
	* Author		: FireSnakeR 										*
	* Created		: 2014-05-07 										*
	...
	*********************************************************************/
	...
	//*** BEGIN PAGE PROCESSING *************************************************//
		...
		if ( count($_POST) > 0 ) //$_POST is always set, so we check by # of element
		{
			...
			if ( isset($_POST['depositVerifySubmit']) && $_POST['depositVerifySubmit'] == 'VERIFY' )
			{
				...
				{
					if (substr_count($key, "depositVerifyID") > 0 )
					{
						//get the counter
						list($key1, $key2) = explode('_', $key);

						$aVerify = array(
							"ID" => $_POST["depositVerifyID_" . $key2],
							"Notes" => $_POST["depositVerifyFinanceNotes_" . $key2],
						);
		...
		if ($_SESSION['user_IsFinance'] == 1)
		{
			...
			$aSelectedOutlet = '';
			foreach ($aOutletList as $aOutlet)
			{
				$aSelectedOutlet .= ' = ' . $aOutlet['ID'] . ' OR outlet_ID';
			}
			$aSelectedOutlet = substr($aSelectedOutlet, 0, strlen($aSelectedOutlet) - 13);

			$aSearchParam = array(
				"(outlet_ID" => $aSelectedOutlet . ")",
				"Date" => "BETWEEN '" . $sBeginDate . "' AND '" . $sEndDate . "'"
			);
		}
		else
	...
	$websiteFiles = array(
		...
		"content" => "finance/depositBank.htm"
	);
	...
	$cWebsite->template->set_var(array(
		...
		"VAR_ACTIONURL" => "finance/depositBank.php",
		//page text
