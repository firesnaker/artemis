<?php
    ...
    * 																		*
    *************************************************************************
	* finance/index.php :: Finance Login Page								*
	*********************************************************************
	* The login page for finance											*
	*																	*
	* Version		: 0.1												*
	* Author		: FireSnakeR 										*
	* Created		: 2012-04-29 										*
	...
	*********************************************************************/
		...
		if ( count($_POST) > 0 ) //$_POST is always set, so we check by # of element
		{
			...
			if ( $cValidator->isValidType($_POST, $aLogin) )
			{
				...
				if ( $sLoginResult > 0 ) //is valid user
				{
					...
					if ( ($cUser->Name == "admin" || $cUser->Name == "FINANCE" || $cUser->IsFinance == 1) )
					{
						$_SESSION['user_ID'] = $cUser->ID;
						$_SESSION['user_Name'] = $cUser->Name;
						$_SESSION['user_Level'] = $cUser->Level;
						$_SESSION['user_IsFinance'] = $cUser->IsFinance;
	...
	$cWebsite->template->set_var(array(
		...
		"VAR_FORM_ACTION" => "finance/index.php",
