<?php
    ...
    * 																		*
    *************************************************************************
	* finance/deposit.php :: Admin Index Page								*
	*********************************************************************
	* The deposit page for finance											*
	*																	*
	* Version		: 0.1												*
	* Author		: FireSnakeR 										*
	* Created		: 2012-05-03 										*
	...
	*********************************************************************/
	...
	//*** BEGIN PAGE PROCESSING *************************************************//
			...
			if ( isset($_POST['depositVerifySubmit']) && $_POST['depositVerifySubmit'] == 'VERIFY' )
			{
				foreach ($_POST as $key => $value)
				{
					if (substr_count($key, "depositVerifyID") > 0 )
					{
						...
						{
							$cDeposit->Verify($aVerify["ID"], $aVerify["Notes"]);
						}
					}
				}
			}
		...
		if ($_SESSION['user_IsFinance'] == 1)
		{
			...
			$aDepositList = $cDeposit->GetDepositReportByFinanceArea($aSearchByFieldArray, array(), array());
		}
		else
		{

			$aOutletList = $cOutlet->GetActiveOutletList();
			$aDepositList = $cDeposit->GetDepositReport($aSearchByFieldArray, array(), array());
		}
	//*** END PAGE PROCESSING ***************************************************//
