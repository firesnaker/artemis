<?php
	/********************************************************************
	* mkios/index.php :: Retail Login Page								*
	*********************************************************************
	...
	*********************************************************************/
		...
		if ( count($_POST) > 0 ) //$_POST is always set, so we check by # of element
		{
			...
			if ( $cValidator->isValidType($_POST, $aLogin) )
			{
				...
				if ( $sLoginResult > 0 ) //is valid user
				{
					...
					if ( ( strtolower($cUser->Username) == 'mkios') || ( strtolower($cUser->Username) == 'admin') )
					{
					...
					{
						$sErrorMessages = "Failed authorization!"; //TODO: change into language variables
					}
				}
				else
				{
	...
	$cWebsite->template->set_var(array(
		...
		"VAR_FORM_ACTION" => "mkios/index.php",
